...
class ExtTableLayout(QVBoxLayout):
    ...
    def __init__(self, reference, settings, readonly, parent):
        ...
        self.items = (
            ...
            FormItem(settings, 'price8', self.tr("MS-65"), Type.Money | additional_type),
            FormItem(settings, 'price7', self.tr("MS-63"), Type.Money | additional_type),
...
class PricesTableLayout(QVBoxLayout):
    ...
    def __init__(self, reference, settings, readonly, parent):
        ...
        if readonly:
            ...
            grade_section = reference.section('grade')
            place_section = reference.section('place')

        self.items = (
            FormItem(settings, 'action', self.tr("Action"), Type.String | additional_type),
            FormItem(settings, 'date', self.tr("Date"), Type.Date | additional_type),
            FormItem(settings, 'quantity', self.tr("Quantity"), Type.BigInt | additional_type),
            FormItem(settings, 'price', self.tr("Price"), Type.Money | additional_type),
            FormItem(settings, 'currency', self.tr("Currency"), Type.String | additional_type),
            FormItem(settings, 'total_price', self.tr("Total price"), Type.Money | additional_type),
            FormItem(settings, 'shipping', self.tr("Shipping"), Type.Money | additional_type),
            FormItem(settings, 'grade', self.tr("Grade"), Type.String | additional_type,
                     reference=reference, section=grade_section),
            FormItem(settings, 'url', self.tr("URL"), Type.String | additional_type),
            FormItem(settings, 'place', self.tr("Place"), Type.String | additional_type,
                     reference=reference, section=place_section),
            FormItem(settings, 'number', self.tr("#"), Type.String | additional_type),
            FormItem(settings, 'counterparty', self.tr("Counterparty"), Type.String | additional_type),
            FormItem(settings, 'info', self.tr("Info"), Type.Text | additional_type),
            FormItem(settings, 'start_bid', self.tr("Start bid"), Type.Money | additional_type),
        )
    ...
    def delete_record(self):
        if self.current_row != -1:
            key = 'show_warn/confirm_delete_prices'
            settings = QSettings()
            show = settings.value(key, True, type=bool)
            if show:
                msg_box = QMessageBox(QMessageBox.Warning, self.tr("Delete"),
    ...
    def fill(self, record):
        ...
        row_idx = 0
        prices_data = record.value('prices')
